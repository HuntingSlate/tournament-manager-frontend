import type { FC } from 'react';

import { Group, Flex, Stack, Button, Text } from '@mantine/core';
import { IconLink } from '@tabler/icons-react';

import { useRemoveProfileLinkMutation } from '@/src/api/mutations/userMutations';
import { vars } from '@/src/theme';

type ProfileLinkProps = {
	id: number;
	type: string;
	url: string;
	platformUsername?: string;
	onEditClick?: () => void;
};

export const ProfileLink: FC<ProfileLinkProps> = ({
	id,
	type,
	url,
	platformUsername,
	onEditClick,
}) => {
	const { mutate, isPending } = useRemoveProfileLinkMutation();

	const handleDeleteLink = () => {
		mutate({ linkId: id });
	};

	return (
		<Group p={16} bdrs={4} bg={vars.colors.white} bd='1px solid #ced4de' justify='space-between'>
			<Group gap={12}>
				<Flex style={{ padding: 8, backgroundColor: vars.colors.gray[4], borderRadius: '50%' }}>
					<IconLink size={20} />
				</Flex>
				<Stack gap={0}>
					<Group>
						<Text size='md' fw={600}>
							{platformUsername ? `${type} - ${platformUsername}` : type}
						</Text>
					</Group>
					<Text size='xs'>{url}</Text>
				</Stack>
			</Group>
			<Group gap={10}>
				<Button variant='subtle' color='blue' size='sm' onClick={onEditClick}>
					Edit
				</Button>
				<Button
					variant='subtle'
					color='red'
					size='sm'
					onClick={handleDeleteLink}
					loading={isPending}
				>
					Delete
				</Button>
			</Group>
		</Group>
	);
};
